...
// which are part of the cryptocause repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="cryptocause-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /cryptocause-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "cryptocausepay-cryptocause/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from cryptocausepay-cryptocause/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'cryptocausepay-cryptocause/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from cryptocausepay-cryptocause/develop"
            }
          }

          if (hasCache) {
            sh "cd /cryptocause-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /cryptocause-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /cryptocause-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /cryptocause-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /cryptocause-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /cryptocause-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /cryptocause-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
