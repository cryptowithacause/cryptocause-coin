...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 CWAC irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on cryptocausecentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in cryptocause.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("cryptocause.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in cryptocause.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "cryptocause",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "cryptocause",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "cryptocause",               "gobject",                &gobject,                true,  {} },
    { "cryptocause",               "voteraw",                &voteraw,                true,  {} },
