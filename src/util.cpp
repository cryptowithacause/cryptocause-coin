...
const char * const BITCOIN_CONF_FILENAME = "cryptocause.conf";
const char * const BITCOIN_PID_FILENAME = "cryptocaused.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "cryptocause" is a composite category enabling all cryptocause-related debug output
                if(ptrCategory->count(std::string("cryptocause"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\cryptocause
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\cryptocause
    // Mac: ~/Library/Application Support/cryptocause
    // Unix: ~/.cryptocause
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "cryptocause";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/cryptocause";
#else
    // Unix
    return pathRet / ".cryptocause";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty cryptocause.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override cryptocause.conf
