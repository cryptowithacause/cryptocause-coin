...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all cryptocause related functionality
...
/*
    ...
    allow endless transaction that would bloat cryptocause and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all cryptocause specific functionality
