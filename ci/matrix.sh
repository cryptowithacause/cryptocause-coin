...
export BUILDER_IMAGE_NAME="cryptocause-builder-$BUILD_TARGET-$JOB_NUMBER"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export USE_SHELL="/bin/cryptocause"
  export PYZMQ=true
  export RUN_TESTS=true
  ...
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_CWAC_DEBUG"
  export PYZMQ=true
  export RUN_TESTS=true
