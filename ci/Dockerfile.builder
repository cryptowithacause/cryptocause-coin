...
# cryptocause_hash
RUN git clone https://github.com/cryptocausepay/cryptocause_hash
RUN cd cryptocause_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} cryptocause
RUN useradd -u ${USER_ID} -g cryptocause -s /bin/bash -m -d /cryptocause cryptocause
...
RUN mkdir /cryptocause-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /cryptocause-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /cryptocause-src
